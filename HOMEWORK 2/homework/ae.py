...
class PatchAutoEncoder(torch.nn.Module, PatchAutoEncoderBase):
    ...
    class PatchEncoder(torch.nn.Module):
        ...
        def __init__(self, patch_size: int, latent_dim: int, bottleneck: int):
            ...
            self.encoder = torch.nn.Sequential(
                ...
                torch.nn.GELU(),
                torch.nn.Linear(256, bottleneck),
            )

        def forward(self, x: torch.Tensor) -> torch.Tensor:
            # x: (B, H, W, 3)
            patches = self.patchify(x)  # (B, h, w, latent_dim)
            bottleneck = self.encoder(patches)  # (B, h, w, bottleneck)
            return bottleneck
    ...
    class PatchDecoder(torch.nn.Module):
        ...
        def forward(self, x: torch.Tensor) -> torch.Tensor:
            # x: (B, h, w, bottleneck)
            decoded = self.decoder(x)  # (B, h, w, latent_dim)
            recon = self.unpatchify(decoded)  # (B, H, W, 3)
            return recon
